...
- You have a plan from sdd-writing-plans and your human partner
  chose inline execution at the handoff.
- Your harness has no subagent tool (see your runtime's documentation for
  the dispatch tools it supports). Never fabricate a dispatch; run the
  plan here.
...
Prefer sdd-subagent-driven-development when your human partner
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "Plan wrong? Rule and ledger. Code wrong? sdd-systematic-debugging" [shape=box];
    ...
    "Step output matches plan's Expected?" -> "Plan wrong? Rule and ledger. Code wrong? sdd-systematic-debugging" [label="no"];
    "Plan wrong? Rule and ledger. Code wrong? sdd-systematic-debugging" -> "Work the steps in order: TDD, run every verification, read every output";
    ...
    "Spec acceptance: run each Success Criterion, ledger results" -> "Final whole-branch review (fresh reviewer if you have one)";
...
**Scripts.** `SKILL_DIR` below is this skill's directory (containing this
SKILL.md); `SDD_DIR` is its sibling `sdd-subagent-driven-development` skill
directory. Resolve both once and invoke scripts by absolute path —
`bash "$SKILL_DIR/scripts/task-start" ...` — while keeping your CWD at the
...
The workspace and ledger are shared with sdd-subagent-driven-development
...
- Each plan owns a workspace: at skill start, run
  `bash "$SDD_DIR/scripts/sdd-workspace" PLAN_FILE` — it
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its first
  ...
  recollection. A ledger whose first line names a different plan file — or
  a stray ledger at the old flat path `.superpowers/sdd/progress.md` — is
...
**REQUIRED SUB-SKILL:** load sdd-test-driven-development now,
...
produces against what the other consumes, and what you found — plus one
row per task whose text contradicts the plan's Global Constraints. Tasks
that share nothing get no row; a plan whose tasks share nothing gets the
single line `Pre-flight: no shared interfaces`. Rule on each conflict a
row surfaces with the spec as the binding authority, record the ruling
beside its row, and start Task 1. Each task's own internal consistency is
checked when you read its brief, not here.
...
- Run this skill's `bash "$SKILL_DIR/scripts/task-start" PLAN_FILE N`. It prints the brief
...
order under sdd-test-driven-development, loaded at setup. A test
...
- **Matches.** Next step.
- **The code is wrong.** Use sdd-systematic-debugging. Find the
...
**REQUIRED SUB-SKILL:** sdd-verification-before-completion governs
the claim. If any item is missing, the task is not complete: finish it.

### 4. Complete the task

Run this skill's `bash "$SKILL_DIR/scripts/task-done" PLAN_FILE N BASE -- <test command>`
...
Then run the spec acceptance pass the per-task contract does not cover.
Re-read the spec's Confirmed Intent and Success Criteria and work each
criterion:
...
Findings here enter the one fix pass below.

## Final Review

Confirm `git status --porcelain -uno` is still empty, then run
`bash "$SDD_DIR/scripts/review-package" PLAN_FILE MERGE_BASE HEAD`
(MERGE_BASE = the commit the branch started from, e.g.
`git merge-base main HEAD`; exit 3 means no package — an empty range means
the work is uncommitted, an empty net diff means nothing changed: ledger a
ruling) and review from the file it prints.

**With a subagent tool:** dispatch the `sdd-final-reviewer` agent — the
whole-branch review is a judgment task, and the agent definition pins its
model and thinking variant — using
sdd-requesting-code-review's
[code-reviewer.md](../sdd-requesting-code-review/code-reviewer.md), with the
package path, the plan and spec paths, the plan's Review Focus section
verbatim if it has one (the spec-silent cases the plan's tests pin — the
reviewer checks each deliberately), and a pointer to the ledger's
spec-acceptance and `Ruling:` lines so it can weigh the calls you made.
Fill the template's inputs explicitly: `[DESCRIPTION]`,
`[PLAN_OR_REQUIREMENTS]` (plan and spec paths), `[BASE_SHA]` (MERGE_BASE),
`[HEAD_SHA]`, and `[DIFF_FILE]` (the package path). This is the one fresh
context the whole run buys. Do not
...
Sort the findings within each axis before you act on any of them. Spec and
Standards stay separate in the reviewer's report — never merge or rerank
them into a single list; severity is the gate you apply to findings from
both axes. The reviewer's severity labels are advice; the gate is yours.
Its "Declined to judge" list is yours too: every line there is a ruling
you make and ledger, exactly like
...
```
You: I'm using the sdd-executing-plans skill to implement this plan inline.

[Setup: worktree verified; plan + spec read; sdd-workspace resolved;
 pre-flight scan written to ledger; a todo per task]

Task 1: Hook installation script
[task-start plan 1 → brief read; BASE a1b2c3d]
[Steps in RED-GREEN order: failing test → FAIL as Expected → implement →
 PASS 1/1 → commit d4e5f6a]
[task-done plan 1 a1b2c3d -- npm test -- hooks → ledger:
 Task 1: complete (commits a1b2c3d..d4e5f6a, tests: npm test -- hooks → 1/1 pass)]

Task 2: Recovery modes
[task-start plan 2 → brief consumes install_hook; Task 1 Produces installHook]
[Ruling: brief's consumer name is a typo against Task 1's Produces; use
 installHook — cost if wrong: one rename]
[Steps → commit b7c8d9e; task-done plan 2 → 8/8 pass]
...
[After all tasks: spec acceptance — each Success Criterion's command or
 ...
 final review; re-grade findings by effect; Critical/Important → ONE fix
 pass, each fix RED→GREEN plus a green suite; Minor → ledger as deferred]
...
Using sdd-finishing-a-development-branch.
```
