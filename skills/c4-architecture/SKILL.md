...
4. **Write source** - Write `.md` source files to `docs/architecture/`
5. **Build HTML** - Run `build.py` to generate self-contained interactive HTML files in `docs/architecture/html/`
6. **Open in browser** - Open the `.html` files so the user sees rendered, interactive diagrams

### The build pipeline

The `.md` files are the **source of truth** — edit these, never edit `.html` directly.
`docs/architecture/html/build.py` converts them to self-contained HTML:

- Pandoc converts prose (text, lists, tables) to HTML
- Mermaid code blocks are extracted as **raw text** (no HTML encoding) so Mermaid.js gets clean input
- Each diagram is wrapped in an interactive container with:
  - **Zoom** — `+`/`−` buttons + mouse wheel (range 20%–500%, cursor-centered)
  - **Pan** — click + drag
  - **Resize** — drag handles on bottom edge, right edge, and bottom-right corner
  - **Auto-fit** — diagram scales to fill the viewport on render and on every resize
  - **Reset** — `↺` button re-fits the diagram to the current viewport
- Mermaid.js loaded from CDN (v11, dark theme)
- Navigation bar links between all diagrams in the set

To regenerate after editing `.md` files:
```bash
python3 docs/architecture/html/build.py
```
...
Write `.md` source files to `docs/architecture/` with naming convention:
...
Generated HTML lives in `docs/architecture/html/` (built by `build.py`):
- `c4-context.html`
- `c4-containers.html`
- `c4-components-{feature}.html`
- `c4-deployment.html`
- `c4-dynamic-{flow}.html`

**Do not edit `.html` files directly.** Edit the `.md` source, then re-run `build.py`.

When adding a new diagram, also add its filename to the nav bar in `build.py`'s `TEMPLATE` so the user can navigate between all diagrams.
