...
### Setup on this machine (verified 2026-09-22)

Installed globally as `playwright-cli` 0.1.21 (nvm prefix, no `sudo`). Two quirks in this environment — apply both or `open` fails:

**1. Always pass `--browser=chromium`.** The default browser is branded Google Chrome, which is not installed here, so a bare `playwright-cli open` fails with `Chromium distribution 'chrome' is not found at /Applications/Google Chrome.app/...`. The bundled `chrome-for-testing` build (chromium-1246) is installed in `~/Library/Caches/ms-playwright`, so `--browser=chromium` works. To make the bare default work instead, run `npx playwright install chrome`.

**2. Add `PWTEST_SOCKETS_DIR=/tmp/pw` if you see a socket path error.** In the agent terminal this fails with `Socket directory path is too long (102 bytes); set PWTEST_SOCKETS_DIR to a shorter location`, because the terminal's `TMPDIR` is unusually long (`/private/var/folders/.../T/zed-agent-terminal-XXXXXX`). A normal terminal's shorter `TMPDIR` probably does not need this.

```bash
PWTEST_SOCKETS_DIR=/tmp/pw playwright-cli open --browser=chromium https://example.com/
PWTEST_SOCKETS_DIR=/tmp/pw playwright-cli --raw eval "document.title"
PWTEST_SOCKETS_DIR=/tmp/pw playwright-cli close
```

Each command is a separate process, so set the variable per command (or export it once in the shell). Commands also create a `.playwright-cli/` folder holding snapshot files in the current directory — remove it afterwards unless that directory is scratch space.
