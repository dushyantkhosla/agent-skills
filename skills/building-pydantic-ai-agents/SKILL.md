...
### Create a basic Agent with local provider

This example uses the model `qwen3.5-9b-mlx` running on LMStudio

```python
from pydantic_ai import Agent, ModelSettings
from pydantic_ai.models.openai import OpenAIChatModel
from pydantic_ai.providers.openai import OpenAIProvider
from src import my_prompt, my_pydantic_model

agent = Agent(
    model=OpenAIChatModel(
        'qwen3.5-9b-mlx',
        provider=OpenAIProvider(base_url="http://localhost:1234/v1", api_key="x"),
    ),
    model_settings=ModelSettings(thinking=False),
    output_type=my_pydantic_model,
    output_retries=2,
